...
def procesar_videos():
    ...
    def worker():
        try:
            ...
            video_pred = os.path.join(OUTPUT_DIR, "EdgeImpulse17_linea_tiempo_monitoreo.mp4")
            video_accel = os.path.join(OUTPUT_DIR, "EdgeImpulse17_acelerometro_monitoreo.mp4")
            ...
            root.after(0, lambda: lbl_status.config(text="Generando video 1/2 (Gráfico de clases)...", fg="blue"))
            generar_videos.generar_video_predicciones(data, video_pred)
            
            # Avisar que empezo el segundo
            root.after(0, lambda: lbl_status.config(text="Generando video 2/2 (Acelerómetro)...", fg="blue"))
            generar_videos.generar_video_acelerometro(data, video_accel)
            
            root.after(0, lambda: lbl_status.config(text="Éxito: Videos generados correctamente.", fg="green"))
            ...
            root.after(0, lambda: lbl_status.config(text="Ocurrió un error.", fg="red"))
...
root = tk.Tk()
root.title("Reconstrucción Visual — Edge Impulse 17 Clases")
# Permitir auto-ajuste al DPI de la pantalla eliminando root.geometry
...
tk.Label(main_frame, text="Herramienta de Videos — Edge Impulse (17 Clases)", font=font_title, bg="#f4f4f9", fg="#333").pack(pady=(0, 20))

instrucciones = (
    "Sigue estos pasos:\n\n"
    "1. Ve a la carpeta que se acaba de crear llamada 'input_json'.\n"
    "2. Coloca ahi el archivo JSON exportado de la aplicación (solo 1).\n"
    "   Nombre esperado: datos-monitoreo-edgeimpulse17-clases.json\n"
